...
from pretrain_apn import random_sample, save_img, clean, log, build_gif, log_acc
...
def test(net, dataloader):
    ...
    correct_summary = {'clsf-0': {'top-1': 0, 'top-5': 0},
                       'clsf-1': {'top-1': 0, 'top-5': 0},
                       'clsf-2': {'top-1': 0, 'top-5': 0},
                       'clsf-3': {'top-1': 0, 'top-5': 0},}
            ...
            if step > 200:
                for clsf in correct_summary.keys():
                    ...
                    for topk in _summary.keys():
                        log_acc(f'\tAccuracy {clsf}@{topk} ({step}/{len(dataloader)}) = {_summary[topk]/((step+1)*int(inputs.shape[0])):.5%}')
                return
...
if __name__ == "__main__":
    ...
    run(pretrained_model='build/racnn_pretrained-1652536553.pt')
