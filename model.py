...
class RACNN(nn.Module):
    def __init__(self, num_classes, img_scale=448):
        ...
        self.scale_linear = nn.Linear(960, 1000)
    ...
    def forward(self, x):
        ...
        pred = torch.cat([pool_s1.view(-1, 320), pool_s2.view(-1, 320), pool_s3.view(-1, 320)], dim=1)
        pred = self.scale_linear(pred)
        pred = self.leaky_relu(pred)
        pred = self.classifier(pred)
        return [pred1, pred2, pred3, pred], [feature_s1, feature_s2], [attention_s1, attention_s2], [resized_s1, resized_s2]
